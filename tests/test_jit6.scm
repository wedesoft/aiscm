...
(ok (equal? 42 ((jit ctx '() (const (native-call <int> '() jit-constant-fun)))))
    "Compile method call to function returning constant value")
(ok (equal? 63 ((jit ctx (list <int>) (lambda (x) (+ x (native-call <int> (list <int>) jit-constant-fun)))) 21))
    "Compile function call and plus operation to test that caller-saved registers get blocked")
(ok (equal? 2 ((jit ctx (list <int> <int>) (lambda (x y) (native-call <int> (list <int> <int>) jit-subtracting-fun y x))) 5 7))
    "Compile function call taking two arguments")
(ok (equal? 5 ((jit ctx (make-list 3 <int>) (lambda (x y z) (native-call <int> (list <int> <int>) jit-subtracting-fun x (+ y z)))) 10 2 3))
    "Pass result of expression to function call")
(ok (equal? 42 ((jit ctx (list <int> <int>) (lambda (a b) (native-call <int> (make-list 7 <int>) jit-seven-arguments a a a a a a b))) 123 42))
    "Compile function call with seven arguments (requires stack parameters)")
(ok (equal? '(#t #f) (map (jit ctx (list <bool>) (cut native-call <bool> (list <bool>) jit-boolean-not <>)) '(#f #t)))
    "Compile and run native boolean negation function")
(ok (equal? 42 ((jit ctx (list <int>) (cut native-call <int> (list <int>) cabs <>)) -42))
