...
(ok (equal? (list (MOV CL AL) (MOVZX AX CL) (RET))
            (register-allocate (append ((code (typecast <sint> (parameter u))) w) (list (RET)))))
...
(ok (equal? '(253 252 250) (to-list ((jit ctx (list (sequence <ubyte>)) ~) (seq 2 3 5))))
    "Bitwise negation of sequence")
(ok (equal? '(-1 2 -3) (to-list ((jit ctx (list (sequence <int>)) -) (seq <int> 1 -2 3))))
    "Negate sequence of integers")
(ok (equal? '(-1 2 3) (to-list ((jit ctx (list (sequence <byte>)) (cut typecast <int> <>))
                                (seq <byte> -1 2 3))))
