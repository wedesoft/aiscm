...
(define l2 (random (ash 1 60)))
(define ctx (make <jit-context>))
(define mem  (make <mem> #:size 16))
(define ptr  (make (pointer <int>) #:value mem))
(store ptr (make <int> #:value i1))
(store (+ ptr 1) (make <int> #:value i2))
...
(ok (eqv? (+ i1 i2) ((asm ctx int
                          (list (MOV R10W i1)
                                ...
                                (MOV EAX R10W)
                                (RET)))))
    "Function using R10W to add two integers")
...
(ok (eqv? (+ i1 i2) ((asm ctx int
                          (list (MOV R14W i1)
                                ...
                                (MOV EAX R14W)
                                (RET)))))
    "Function using R14W and R15W to add two integers")
(ok (equal? '(#x90) (NOP))
    "NOP # no operation")
...
(ok (eqv? i1 ((asm ctx int (list (MOV RCX mem) (MOV EAX *RCX) (RET)))))
    "Function loading integer from address in RCX")
(ok (eqv? i1 ((asm ctx int (list (MOV R10D mem) (MOV EAX *R10D) (RET)))))
    "Function loading integer from address in R10D")
(ok (eqv? i1 ((asm ctx int (list (MOV RCX mem) (MOV EAX *RCX) (RET)))))
    "Function loading value from address given as pointer")
(ok (eqv? i2 ((asm ctx int (list (MOV RCX mem) (MOV EAX *RCX 4) (RET)))))
    "Load value from address in RCX with offset")
(ok (eqv? i2 ((asm ctx int (list (MOV R9D mem) (MOV EAX *R9D 4) (RET)))))
