...
(let [(out  (expression (skeleton (sequence <int>))))
      (incr (var <long>))
      (p    (var <long>))]
  ...
  (ok (equal? p (value (body out p)))
      "body of loop should be rebased to the pointer")
  (ok (is-a? (body out p) (pointer <int>))
      "body of array loop should be a pointer object"))
(let [(in   (expression (skeleton (sequence <int>))))
      (incr (var <long>))
      (p    (var <long>))]
  (ok (equal? (list (IMUL incr (stride (term in)) (size-of (typecode (term (term in)))))
                    (MOV p (get (term (term in)))))
              (setup in incr p))
      "setup of array loop for basic tensor expression should define increment and initialise pointer")
  (ok (equal? (list (ADD p incr))
              (increment incr p))
...
(let [(out (skeleton (sequence <int>)))
  ...
  (ok (list? (code (expression out) (expression in)))
...
(ok (+ (expression (skeleton (sequence <int>))) (skeleton <int>))
    "create function from tensor and element")
(ok (+ (skeleton <int>) (expression (skeleton (sequence <int>))))
...
(let* [(a    (skeleton <int>))
       (b    (skeleton (sequence <int>)))
       (f    (+ (expression a) (expression b)))
       (out  (skeleton (sequence <int>)))
       (incr (var <long>))
       (p    (var <long>))
       (q    (var <long>))]
  (ok (equal? (list (IMUL incr (stride b) (size-of (typecode b)))
                    (MOV q (value b)))
              (setup f incr q))
      "setup of loop over scalar-array-function should setup looping over second argument")
  (ok (equal? a (car (arguments (body f q))))
...
;(skip (eq? <fragment<int<>>> (super (fragment <int>)))
...
;(skip (eq? <int> (type (class-of (parameter (skeleton <int>)))))
;    "Check type of basic fragment wrapping variable")
;(skip (null? (code (parameter (skeleton <int>))))
...
;(skip (eq? <int> (type (class-of (+ (parameter (skeleton <int>)) (parameter (skeleton <int>))))))
;    "Adding two integers returns an integer")
;(skip (eq? <sint> (type (class-of (+ (parameter (skeleton <byte>)) (parameter (skeleton <sint>))))))
...
;(skip (eq? <var> (class-of (get (skeleton <int>))))
;    "'skeleton' creates a variable for an integer type")
;(skip (equal? <int> (class-of (skeleton <int>)))
;    "'skeleton' creates a value of correct integer type")
;(skip (equal? <int> (typecode (get (skeleton <int>))))
;    "'skeleton' creates a variable of correct integer type")
;(skip (eq? (sequence <int>) (class-of (skeleton (sequence <int>))) )
;    "'skeleton' can create sequences")
;(skip (eq? (pointer <int>) (class-of (skeleton (pointer <int>))))
;    "Skeleton of pointer has correct type")
;(skip (eq? <long> (typecode (get (skeleton (pointer <int>)))))
;    "Skeleton of pointer is based on long integer")
;(skip (equal? i1 ((jit ctx (list (pointer <int>)) identity)
;                (make (pointer <int>) #:value (idata))))
;    "Compile and run code for fetching data from a pointer")
;(skip (equal? <long> (typecode (value (skeleton (sequence <byte>)))))
;    "Pointer of sequence skeleton is a long integer")
;(skip (equal? (list <long>) (map typecode (shape (skeleton (sequence <int>)))))
;    "Shape of sequence is a list of long integers")
;(skip (equal? (list <long>) (map typecode (strides (skeleton (sequence <int>)))))
;    "Strides of sequence is a list of long integers")
;(skip (equal? <long> (typecode (value (skeleton (multiarray <byte> 2)))))
;    "Pointer of 2D array skeleton is a long integer")
;(skip (equal? (list <long> <long>) (map typecode (shape (skeleton (multiarray <byte> 2)))))
;    "Shape of 2D array skeleton is a list of long integers")
;(skip (equal? (list <long> <long>) (map typecode (strides (skeleton (multiarray <byte> 2)))))
;    "Strides of 2D array skeleton is a list of long integers")
;(skip (equal? <int> (type (project (parameter (skeleton (sequence <int>))))))
;    "A code fragment returning a sequence can be projected to an element")
;(skip (is-a? ((jit ctx (list (sequence <int>)) identity) (seq <int> 1 2 3)) (sequence <int>))
;    "Function returning sequence allocates return value")
;(skip (equal? '(3) (shape ((jit ctx (list (sequence <int>)) identity) (seq <int> 1 2 3))))
;    "Function returning sequence determines shape of result")
;(skip (equal? '(1 2 3) (to-list ((jit ctx (list (sequence <int>)) identity) (seq <int> 1 2 3))))
...
;(skip (equal? '(-1 2 -3) (to-list ((jit ctx (list (sequence <int>)) -) (seq <int> 1 -2 3))))
...
;(skip (equal? '(#t #t #f #t) (to-list ((jit ctx (list (sequence <int>)) !=0) (seq <int> 1 -2 0 3))))
...
;(skip (equal? '(0 1 2) (to-list ((jit ctx (list <byte> (sequence <int>)) +) -1 (seq <int> 1 2 3))))
;    "Add integer sequence to byte")
;(skip (equal? '(0 6 1) (to-list ((jit ctx (list (sequence <int>) (sequence <int>)) +)
