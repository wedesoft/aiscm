...
(ok (equal? (list (ADD EDI 1) (MOV EDI (ptr <int> RSP 8)) (ADD EDI 2) (MOV (ptr <int> RSP 8) EDI) (RET))
            (pass-parameter-variables <null> (make-list 7 <int>)
                                      ...
                                      #:registers (list RDI)))
    "'pass-parameter-variables' maps the 7th integer parameter correctly")
(ok (equal? (MOV AX 0) (env [] (MOV AX 0)))
    "'env' with empty list of variables has no effect")
(ok (eq? 'x (slot-ref (env [(x <byte>)] x) 'symbol))
    "'env' defines named variable objects")
(ok (eq? <byte> (slot-ref (env [(x <byte>)] x) 'type))
...
(ok (equal? (list (MOV EAX EDI))
            (substitute-variables ((code (parameter a)) b)
                                  (list (cons b RAX) (cons a RDI))))
    "The code for a trivial expression is a copy operation")
...
(ok (equal? (list (MOV CL AL) (MOVZX AX CL) (RET))
            (register-allocate (append ((code (to-type <sint> (parameter u))) w) (list (RET)))))
...
(ok (equal? (list (MOV CL DIL) (MOVZX EAX CL) (RET))
            (assemble a (list u) (to-type <int> (parameter u))))
    "unsigned type conversion")
(ok (equal? (list (MOV CL DIL) (MOVSX EAX CL) (RET))
...
(ok (equal? (list (MOV ECX EDI) (MOV EAX ECX) (MOV ECX ESI) (MOV EDX ECX) (ADD EAX EDX) (RET))
            (assemble c (list a b) (+ (parameter a) (parameter b))))
    "Add two integers")
(ok (equal? (list (MOV CL DIL) (MOVSX EAX CL) (MOV ECX ESI) (MOV EDX ECX) (ADD EAX EDX) (RET))
            (assemble c (list v b) (+ (parameter v) (parameter b))))
    "Add byte and integer")
(ok (equal? (list (MOV ECX EDI) (MOV EAX ECX) (MOV CL SIL) (MOVZX EDX CL) (ADD EAX EDX) (RET))
...
(ok (equal? (list (MOV CX DI) (MOV AX CX) (MOV CL SIL) (MOVSX DX CL) (ADD AX DX) (RET))
...
(ok (equal? (list (MOV EAX (ptr <int> EDI)))
            (substitute-variables ((code (parameter *p)) a) (list (cons p RDI) (cons a RAX))))
    "The code for a trivial pointer expression fetches the value")
(ok (equal? (list (MOV EAX (ptr <int> EDI)) (RET))
            (assemble a (list *p) (parameter *p)))
    "Fetching data from a pointer")
(ok (equal? (list (MOV EAX ESI) (MOV (ptr <int> EDI) EAX) (RET))
