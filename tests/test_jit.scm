...
(ok (unspecified? ((compile ctx <null> '() (lambda () (list (RET))))))
    "'compile' can create an empty function")
(ok (eqv? i1 ((compile ctx <int> '() (lambda (r) (list (MOV r i1) (RET))))))
    "'compile' creates constant function returning an integer")
(ok (eqv? 5 ((compile ctx <ubyte> (list <ubyte> <ubyte>)
                      (lambda (sum x y) (list (MOV sum x) (ADD sum y) (RET)))) 2 3))
    "'compile' creates variables for integer parameters and return values")
(ok (equal? '(2 3 5 7 11 13)
            (map (lambda (i) (apply (compile ctx <int> (make-list 6 <int>)
                                          (lambda (r . args)
                                                  (list (MOV r (list-ref args i)) (RET))))
                   '(2 3 5 7 11 13))) (iota 6)))
    "'compile' maps the first 6 integer parameters correctly")
(ok (eqv? 5 ((compile ctx <long> (list (sequence <ubyte>))
                   (lambda (r s) (list (MOV r (car (shape s))) (RET))))
             (make (sequence <ubyte>) #:size 5)))
    "'compile' composes variables for representing sequences")
(ok (eqv? i1 ((compile ctx <int> (list (pointer <int>)) (lambda (r p) (list (MOV r (ptr <int> p)) (RET))))
              (make (pointer <int>) #:value (idata))))
    "'compile' handles pointer arguments")
(ok (equal?
      '(0 2 3)
      (let [(s (seq 1 2 3))]
        ((compile ctx <null> (list (sequence <ubyte>))
           (lambda (s) (list (MOV (ptr <ubyte> (get-value s)) 0) (RET)))) s)
        (to-list s)))
  "'compile' passes pointer variables for sequence data")
(ok (eqv? 3 ((compile ctx <int> '()  (lambda (r)
                                    (list (MOV r 0) (JMP 'a) (list 'a (MOV r 2)) 'a (ADD r 3) (RET))))))
    "'compile' creates separate namespaces for labels")
...
;(ok (equal? '() (get-code (fragment a)))
;    "The code for an trivial expression is empty")
;(ok (equal? a (get-value (fragment a)))
;    "The value of a trivial expression is itself")
;(ok (equal? <int> (typecode (+ (fragment a) (fragment b))))
;    "Adding two integers returns an integer")
;(ok (equal? (list (MOV ECX EDI) (MOV EDX ESI) (MOV EAX ECX) (ADD EAX EDX) (RET))
;            (build (list a b) (+ (fragment a) (fragment b))))
;    "Building the fragment creates correct assembler code")
;(ok (equal? 9 ((jit ctx (list <int> <int> <int>) +) 2 3 4))
;    "Compiling a plus operation creates an equivalent machine program")
;(ok (equal? <sint> (typecode (typecast <sint> (fragment u)))); TODO: test other typecasts
;    "Conversion to short integer returns a short integer")
;(ok (equal? (list (MOVSX AX DIL) (RET))
;            (build (list u) (typecast <sint> (fragment u))))
;    "Conversion to short integer returns does the required sign extension")
;(ok (equal? <sint> (typecode (+ (fragment u) (fragment w))))
;    "Adding a short integer and a byte returns a byte")
;(ok (equal? (list (MOVSX CX DIL) (MOV DX SI) (MOV AX CX) (ADD AX DX) (RET)); TODO: generate compact code
;            (build (list u w) (+ (fragment u) (fragment w))))
