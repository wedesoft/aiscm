...
(ok (equal? '(#x66 #x8b #xd8) (MOV BX AX))
    "MOV BX, AX")
(ok (equal? '(#x8b #x0a) (MOV ECX (ptr <ptr<32>> RDX)))
    "MOV ECX, [RDX]")
(ok (equal? '(#x48 #x8b #x0a) (MOV RCX (ptr <ptr<64>> RDX)))
    "MOV RCX, [RDX]")
(ok (equal? '(#x41 #x8b #x0b) (MOV ECX (ptr <ptr<32>> R11)))
    "MOV ECX, [R11]")
(ok (equal? '(#x8b #x4a #x04) (MOV ECX (ptr <ptr<32>> RDX 4)))
    "MOV ECX, [RDX] + 4")
(ok (equal? '(#x8b #x8a #x80 #x00 #x00 #x00) (MOV ECX (ptr <ptr<32>> RDX 128)))
    "MOV ECX, [RDX] + 128")
(ok (equal? '(#x8b #x4c #x24 #x04) (MOV ECX (ptr <ptr<32>> RSP 4)))
    "MOV ECX, [RSP] + 4")
(ok (equal? '(#x45 #x8b #x53 #x04) (MOV R10D (ptr <ptr<32>> R11 4)))
    "MOV R10D, [R11] + 4")
(ok (equal? '(#x4d #x8b #x53 #x04) (MOV R10 (ptr <ptr<64>> R11 4)))
    "MOV R10, [R11] + 4")
(ok (equal? '(#x8a #x0a) (MOV CL (ptr <ptr<8>> RDX)))
    "MOV CL, [RDX]")
(ok (equal? '(#x66 #x8b #x0a) (MOV CX (ptr <ptr<16>> RDX)))
    "MOV CX, [RDX]")
(ok (equal? '(#x89 #x11) (MOV (ptr <ptr<32>> RCX) EDX))
    "MOV [RCX], EDX")
(ok (equal? '(#x44 #x89 #x01) (MOV (ptr <ptr<32>> RCX) R8D))
    "MOV [RCX], R8D")
(ok (equal? '(#xc3) (RET))
    "RET # near return")
...
(ok (equal? '(#x49 #x81 #xc2 #x0d #x00 #x00 #x00) (ADD R10 13))
    "ADD R10, 13")
(ok (equal? '(#x80 #x07 #x2a) (ADD (ptr <ptr<8>> RDI) 42))
    "ADD BYTE PTR [RDI], 42")
...
(ok (equal? '(#x02 #xca) (ADD CL DL))
    "ADD CL, DL")
(ok (equal? '(#x03 #x0a) (ADD ECX (ptr <ptr<32>> RDX)))
    "ADD ECX, [RDX]")
...
(ok (equal? '(#x49 #x81 #xea #x0d #x00 #x00 #x00) (SUB R10 13))
    "SUB R10, 13")
(ok (equal? '(#x41 #x81 #x2a #x0d #x00 #x00 #x00) (SUB (ptr <ptr<32>> R10) 13))
    "SUB (ptr <ptr<32>> R10), 13")
...
(ok (eqv? (+ i1 i2) ((asm ctx <int> (list (MOV EAX i2) (MOV RDX (idata)) (ADD EAX (ptr <ptr<32>> RDX)) (RET)))))
    "Add integer memory operand")
(ok (eqv? (+ i1 i2) (begin ((asm ctx <null>
                                 (list (MOV RSI mem)
                                       (MOV (ptr <ptr<32>> RSI) i1)
                                       (ADD (ptr <ptr<32>> RSI) i2)
                                       (RET))))
...
(ok (eqv? i1 ((asm ctx <int> (list (MOV RCX (idata)) (MOV EAX (ptr <ptr<32>> RCX)) (RET)))))
    "Load integer from address in RCX")
(ok (eqv? i1 ((asm ctx <int> (list (MOV R10 (idata)) (MOV EAX (ptr <ptr<32>> R10)) (RET)))))
    "Load integer from address in R10")
(ok (eqv? i2 ((asm ctx <int> (list (MOV RCX (idata)) (MOV EAX (ptr <ptr<32>> RCX 4)) (RET)))))
    "Load integer from address in RCX with offset")
(ok (eqv? i1 ((asm ctx <int> (list (MOV RCX (get-value (+ iptr 50))) (MOV EAX (ptr <ptr<32>> RCX -200)) (RET)))))
    "Load integer from address in RCX with large offset")
(ok (eqv? i2 ((asm ctx <int> (list (MOV R9 (idata)) (MOV EAX (ptr <ptr<32>> R9 4)) (RET)))))
    "Load integer from address in R9D with offset")
(ok (eqv? l1 ((asm ctx <long> (list (MOV RCX (ldata)) (MOV RAX (ptr <ptr<64>> RCX)) (RET)))))
    "Load long integer from address in RCX")
(ok (eqv? l2 ((asm ctx <long> (list (MOV RCX (ldata)) (MOV RAX (ptr <ptr<64>> RCX 8)) (RET)))))
    "Load long integer from address in RCX with offset")
(ok (eqv? w1 ((asm ctx <sint> (list (MOV RCX (wdata)) (MOV AX (ptr <ptr<16>> RCX)) (RET)))))
    "Load short integer from address in RCX")
(ok (eqv? w2 ((asm ctx <sint> (list (MOV RCX (wdata)) (MOV AX (ptr <ptr<16>> RCX 2)) (RET)))))
    "Load short integer from address in RCX with offset")
(ok (eqv? b1 ((asm ctx <byte> (list (MOV RCX (bdata)) (MOV AL (ptr <ptr<8>> RCX)) (RET)))))
    "Load byte from address in RCX")
(ok (eqv? b2 ((asm ctx <byte> (list (MOV RCX (bdata)) (MOV AL (ptr <ptr<8>> RCX 1)) (RET)))))
    "Load byte from address in RCX with offset")
(ok (equal? '(#x48 #x8d #x51 #x04) (LEA RDX (ptr <ptr<64>> RCX 4)))
    "LEA RDX, [RCX + 4]")
(ok (equal? '(#x48 #x8d #x44 #x24 #xfc) (LEA RAX (ptr <ptr<64>> RSP -4)))
    "LEA RAX, [RSP - 4]")
(ok (eqv? i2 ((asm ctx <int> (list (MOV RCX (idata))
                                 (LEA RDX (ptr <ptr<64>> RCX 4))
                                 (MOV EAX (ptr <ptr<32>> RDX))
                                 (RET)))))
    "Load integer from address in RCX with offset using effective address")
(ok (equal? '(#x48 #x8d #x04 #xb7) (LEA RAX (ptr <ptr<64>> RDI RSI (scale 4))))
    "LEA RAX, [RDI + ESI * 4]")
(ok (eqv? i2 ((asm ctx <int> (list (MOV RCX (idata))
                                 (MOV RDI 1)
                                 (LEA RDX (ptr <ptr<64>> RCX RDI (scale 4)))
                                 (MOV EAX (ptr <ptr<32>> RDX))
                                 (RET)))))
    "Load integer from address in RCX with index times 4 using effective address")
(ok (equal? '(#x48 #x8d #x04 #x77) (LEA RAX (ptr <ptr<64>> RDI RSI (scale 2))))
    "LEA RAX, [RDI + ESI * 2]")
(ok (eqv? i2 ((asm ctx <int> (list (MOV RCX (idata))
                                 (MOV RDI 2)
                                 (LEA RDX (ptr <ptr<64>> RCX RDI (scale 2)))
                                 (MOV EAX (ptr <ptr<32>> RDX))
                                 (RET)))))
    "Load integer from address in RCX with index times 2 using effective address")
(ok (equal? '(#x48 #x8d #x44 #x77 #x02) (LEA RAX (ptr <ptr<64>> RDI RSI (scale 2) 2)))
    "LEA RAX, [RDI + ESI * 2 + 2]")
(ok (eqv? i2 ((asm ctx <int> (list (MOV RCX (idata))
                                 (MOV RDI 3)
                                 (LEA RDX (ptr <ptr<64>> RCX RDI (scale 1) 1))
                                 (MOV EAX (ptr <ptr<32>> RDX))
                                 (RET)))))
    "Load integer from address in RCX with index and offset using effective address")
(ok (eqv? i2 ((asm ctx <int> (list (MOV R9 (idata))
                                 (MOV R10 3)
                                 (LEA R11 (ptr <ptr<64>> R9 R10 (scale 1) 1))
                                 (MOV EAX (ptr <ptr<32>> R11))
                                 (RET)))))
    "Load integer from address in R9 with index and offset using effective address")
(ok (eqv? #x08 (begin ((asm ctx <long>
                            (list (MOV RDI (idx))
                                  (MOV AL (ptr <ptr<8>> RDI))
                                  (RET))))))
    "Load 8-bit value from memory")
(ok (eqv? #x0708 (begin ((asm ctx <long>
                              (list (MOV RDI (idx))
                                    (MOV AX (ptr <ptr<16>> RDI))
                                    (RET))))))
    "Load 16-bit value from memory")
(ok (eqv? #x05060708 (begin ((asm ctx <long>
                                  (list (MOV RDI (idx))
                                        (MOV EAX (ptr <ptr<32>> RDI))
                                        (RET))))))
    "Load 32-bit value from memory")
(ok (eqv? #x0102030405060708 (begin ((asm ctx <long>
                                          (list (MOV RDI (idx))
                                                (MOV RAX (ptr <ptr<64>> RDI))
                                                (RET))))))
    "Load 64-bit value from memory")
(ok (eqv? i1 (begin ((asm ctx <null>
                          (list (MOV RSI mem)
                                (MOV ECX i1)
                                (MOV (ptr <ptr<32>> RSI) ECX)
                                (RET))))
                    (get-value (fetch iptr))))
...
(ok (eqv? i1 (begin ((asm ctx <null>
                    ...
                    (get-value (fetch iptr))))
    "Write value of ECX to memory")
(ok (equal? '(#xc7 #x07 #x2a #x00 #x00 #x00) (MOV (ptr <ptr<32>> RDI) 42))
    "MOV DWORD PTR [RDI], 42")
(ok (equal? '(#x48 #xc7 #x07 #x2a #x00 #x00 #x00) (MOV (ptr <ptr<64>> RDI) 42))
    "MOV QWORD PTR [RDI], 42")
(ok (equal? '(#x66 #xc7 #x07 #x2a #x00) (MOV (ptr <ptr<16>> RDI) 42))
    "MOV WORD PTR [RDI], 42")
(ok (equal? '(#xc6 #x07 #x2a) (MOV (ptr <ptr<8>> RDI) 42))
    "MOV BYTE PTR [RDI], 42")
...
(ok (eqv? 17 ((asm ctx <int> (list (MOV EAX (ptr <ptr<32>> RSP #x8)) (RET))
                   <int> <int> <int> <int> <int> <int> <int> <int>) 2 3 5 7 11 13 17 19))
    "Function return seventh integer argument")
(ok (eqv? 19 ((asm ctx <int> (list (MOV EAX (ptr <ptr<32>> RSP #x10)) (RET))
                   <int> <int> <int> <int> <int> <int> <int> <int>) 2 3 5 7 11 13 17 19))
...
(ok (eqv? (- w1) ((asm ctx <sint> (list (MOV AX DI) (NEG AX) (RET)) <sint>) w1))
    "Function negating a short integer")
(ok (eqv? (- b1) ((asm ctx <byte> (list (MOV AL DIL) (NEG AL) (RET)) <byte>) b1))
...
(ok (equal? '(#x41 #x0f #x94 #xc1) (SETE R9L))
    "SETE R9L")
(ok (eqv? 1 ((asm ctx <byte> (list (MOV EAX EDI) (CMP EAX 0) (SETE AL) (RET)) <int>) 0))
    "Compare zero in EAX with zero")
(ok (eqv? 0 ((asm ctx <byte> (list (MOV EAX EDI) (CMP EAX 0) (SETE AL) (RET)) <int>) (logior 1 i1)))
    "Compare non-zero number in EAX with zero")
(ok (equal? '(#x41 #x81 #xfa #x2a #x00 #x00 #x00) (CMP R10D 42))
    "CMP R10D 42")
(ok (eqv? 1 ((asm ctx <byte> (list (MOV R10D EDI) (CMP R10D 0) (SETE AL) (RET)) <int>) 0))
    "Compare zero in R10D with zero")
(ok (eqv? 0 ((asm ctx <byte> (list (MOV R10D EDI) (CMP R10D 0) (SETE AL) (RET)) <int>) (logior 1 i1)))
    "Compare non-zero number in R10D with zero")
(ok (equal? '(#x3b #xfe) (CMP EDI ESI))
    "CMP EDI ESI")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETE AL) (RET)) <int> <int>) i1 i1))
    "Two integers being equal")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETE AL) (RET)) <int> <int>) i1 (logxor 1 i1)))
    "Two integers not being equal")
(ok (equal? '(#x48 #x3b #xf7) (CMP RSI RDI))
    "CMP RSI RDI")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP RSI RDI) (SETE AL) (RET)) <long> <long>) l1 l1))
    "Two long integers being equal")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP RSI RDI) (SETE AL) (RET)) <long> <long>) l1 (logxor 1 l1)))
    "Two long integers not being equal")
(ok (equal? '(#x41 #x0f #x92 #xc1) (SETB R9L))
    "SETB R9L")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETB AL) (RET)) <uint> <uint>) 1 3))
    "Unsigned integer being below another")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETB AL) (RET)) <uint> <uint>) 3 3))
    "Unsigned integer not being below another")
(ok (equal? '(#x41 #x0f #x93 #xc1) (SETNB R9L))
    "SETNB R9L")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETNB AL) (RET)) <uint> <uint>) 1 3))
    "Unsigned integer not being above or equal")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETNB AL) (RET)) <uint> <uint>) 3 3))
    "Unsigned integer being above or equal")
(ok (equal? '(#x41 #x0f #x95 #xc1) (SETNE R9L))
    "SETNE R9L")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETNE AL) (RET)) <int> <int>) i1 i1))
    "Two integers not being unequal")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETNE AL) (RET)) <int> <int>) i1 (logxor 1 i1)))
    "Two integers being unequal")
(ok (equal? '(#x41 #x0f #x96 #xc1) (SETBE R9L))
    "SETBE R9L")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETBE AL) (RET)) <uint> <uint>) 3 3))
    "Unsigned integer being below or equal")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETBE AL) (RET)) <uint> <uint>) 4 3))
    "Unsigned integer not being below or equal")
(ok (equal? '(#x41 #x0f #x97 #xc1) (SETNBE R9L))
    "SETNBE R9L")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETNBE AL) (RET)) <uint> <uint>) 3 3))
    "Unsigned integer not being above")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETNBE AL) (RET)) <uint> <uint>) 4 3))
    "Unsigned integer being above")
(ok (equal? '(#x41 #x0f #x9c #xc1) (SETL R9L))
    "SETL R9L")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETL AL) (RET)) <int> <int>) -2 3))
    "Signed integer being less")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETL AL) (RET)) <int> <int>) 3 3))
    "Signed integer not being less")
(ok (equal? '(#x41 #x0f #x9d #xc1) (SETNL R9L))
    "SETNL R9L")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETNL AL) (RET)) <int> <int>) -2 3))
    "Signed integer not being greater or equal")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETNL AL) (RET)) <int> <int>) 3 3))
    "Signed integer being greater or equal")
(ok (equal? '(#x41 #x0f #x9e #xc1) (SETLE R9L))
    "SETLE R9L")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETLE AL) (RET)) <int> <int>) -2 -2))
    "Signed integer being less or equal")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETLE AL) (RET)) <int> <int>) 3 -2))
    "Signed integer not being less or equal")
(ok (equal? '(#x41 #x0f #x9f #xc1) (SETNLE R9L))
    "SETNLE R9L")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETNLE AL) (RET)) <int> <int>) -2 -2))
    "Signed integer not being greater")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETNLE AL) (RET)) <int> <int>) 3 -2))
...
(ok (eqv? 42 ((asm ctx <int> (list (MOV (ptr <ptr<64>> RSP -8) RBX)
                                   (SUB RSP 8)
                                   (MOV EBX 42)
                                   (MOV EAX EBX)
                                   (MOV RBX (ptr <ptr<64>> RSP))
                                   (ADD RSP 8)
                                   (RET)))))
