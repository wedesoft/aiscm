...
(ok (eqv? 0 (assq-ref (labels->offsets (list (label test))) 'test))
    "Sole label maps to zero")
(ok (eqv? 0 (assq-ref (labels->offsets (list (label test) (NOP))) 'test))
    "Label at beginning of code is zero")
(ok (eqv? (length (MOV EAX ESI))
          (assq-ref (labels->offsets (list (MOV EAX ESI) (label test))) 'test))
    "Label after MOV EAX ESI statement maps to length of that statement")
(ok (equal? '(1 2) (let ((a (labels->offsets (list (NOP) (label x) (NOP) (label y)))))
