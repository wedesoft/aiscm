...
(ok (eqv? (+ i1 i2) ((asm ctx <int> (list (MOV R14D i1) (MOV R15D i2) (ADD R14D R15D) (MOV EAX R14D)))))
...
(ok (eqv? (+ i1 i2) ((asm ctx <int> (list (MOV EAX i2) (MOV RDX (idata)) (ADD EAX (ptr <int> RDX))))))
...
(ok (eqv? i1 ((asm ctx <int> (list (MOV RCX (get-value (+ iptr 50))) (MOV EAX (ptr <int> RCX -200))))))
...
(ok (equal? '(#x48 #x8d #x04 #xb7) (LEA RAX (ptr <int> RDI RSI (scale 4))))
    "LEA RAX, [RDI + ESI * 4]")
(ok (eqv? i2 ((asm ctx <int> (list (MOV RCX (idata))
                                 (MOV RDI 1)
                                 (LEA RDX (ptr <int> RCX RDI (scale 4)))
                                 (MOV EAX (ptr <int> RDX))))))
    "Load integer from address in RCX with index times 4 using effective address")
(ok (equal? '(#x48 #x8d #x04 #x77) (LEA RAX (ptr <sint> RDI RSI (scale 2))))
    "LEA RAX, [RDI + ESI * 2]")
(ok (eqv? i2 ((asm ctx <int> (list (MOV RCX (idata))
                                 (MOV RDI 2)
                                 (LEA RDX (ptr <sint> RCX RDI (scale 2)))
                                 (MOV EAX (ptr <int> RDX))))))
    "Load integer from address in RCX with index times 2 using effective address")
(ok (equal? '(#x48 #x8d #x44 #x77 #x02) (LEA RAX (ptr <long> RDI RSI (scale 2) 2)))
    "LEA RAX, [RDI + ESI * 2 + 2]")
(ok (eqv? i2 ((asm ctx <int> (list (MOV RCX (idata))
                                 (MOV RDI 3)
                                 (LEA RDX (ptr <byte> RCX RDI (scale 1) 1))
                                 (MOV EAX (ptr <int> RDX))))))
    "Load integer from address in RCX with index and offset using effective address")
(ok (eqv? i2 ((asm ctx <int> (list (MOV R9 (idata))
                                 (MOV R10 3)
                                 (LEA R11 (ptr <byte> R9 R10 (scale 1) 1))
                                 (MOV EAX (ptr <int> R11))))))
...
(ok (equal? '(#xf6 #xdb) (NEG BL))
    "NEG BL")
(ok (eqv? (- i1) ((asm ctx <int> (list (MOV EAX EDI) (NEG EAX)) <int>) i1))
    "Function negating an integer")
(ok (eqv? (- l1) ((asm ctx <long> (list (MOV RAX RDI) (NEG RAX)) <long>) l1))
    "Function negating a long integer")
(ok (eqv? (- w1) ((asm ctx <sint> (list (MOV AX DI) (NEG AX)) <sint>) w1))
    "Function negating a short integer")
(ok (eqv? (- b1) ((asm ctx <byte> (list (MOV AL DIL) (NEG AL)) <byte>) b1))
...
(ok (equal? '(#x41 #x0f #x94 #xc1) (SETE R9L))
    "SETE R9L")
(ok (eqv? 1 ((asm ctx <byte> (list (MOV EAX EDI) (CMP EAX 0) (SETE AL)) <int>) 0))
    "Compare zero in EAX with zero")
(ok (eqv? 0 ((asm ctx <byte> (list (MOV EAX EDI) (CMP EAX 0) (SETE AL)) <int>) (logior 1 i1)))
    "Compare non-zero number in EAX with zero")
(ok (equal? '(#x41 #x81 #xfa #x2a #x00 #x00 #x00) (CMP R10D 42))
    "CMP R10D 42")
(ok (eqv? 1 ((asm ctx <byte> (list (MOV R10D EDI) (CMP R10D 0) (SETE AL)) <int>) 0))
    "Compare zero in R10D with zero")
(ok (eqv? 0 ((asm ctx <byte> (list (MOV R10D EDI) (CMP R10D 0) (SETE AL)) <int>) (logior 1 i1)))
    "Compare non-zero number in R10D with zero")
(ok (equal? '(#x3b #xfe) (CMP EDI ESI))
    "CMP EDI ESI")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETE AL)) <int> <int>) i1 i1))
    "Two integers being equal")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETE AL)) <int> <int>) i1 (logxor 1 i1)))
    "Two integers not being equal")
(ok (equal? '(#x48 #x3b #xf7) (CMP RSI RDI))
    "CMP RSI RDI")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP RSI RDI) (SETE AL)) <long> <long>) l1 l1))
    "Two long integers being equal")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP RSI RDI) (SETE AL)) <long> <long>) l1 (logxor 1 l1)))
    "Two long integers not being equal")
(ok (equal? '(#x41 #x0f #x92 #xc1) (SETB R9L))
    "SETB R9L")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETB AL)) <uint> <uint>) 1 3))
    "Unsigned integer being below another")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETB AL)) <uint> <uint>) 3 3))
    "Unsigned integer not being below another")
(ok (equal? '(#x41 #x0f #x93 #xc1) (SETNB R9L))
    "SETNB R9L")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETNB AL)) <uint> <uint>) 1 3))
    "Unsigned integer not being above or equal")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETNB AL)) <uint> <uint>) 3 3))
    "Unsigned integer being above or equal")
(ok (equal? '(#x41 #x0f #x95 #xc1) (SETNE R9L))
    "SETNE R9L")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETNE AL)) <int> <int>) i1 i1))
    "Two integers not being unequal")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETNE AL)) <int> <int>) i1 (logxor 1 i1)))
    "Two integers being unequal")
(ok (equal? '(#x41 #x0f #x96 #xc1) (SETBE R9L))
    "SETBE R9L")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETBE AL)) <uint> <uint>) 3 3))
    "Unsigned integer being below or equal")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETBE AL)) <uint> <uint>) 4 3))
    "Unsigned integer not being below or equal")
(ok (equal? '(#x41 #x0f #x97 #xc1) (SETNBE R9L))
    "SETNBE R9L")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETNBE AL)) <uint> <uint>) 3 3))
    "Unsigned integer not being above")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETNBE AL)) <uint> <uint>) 4 3))
    "Unsigned integer being above")
(ok (equal? '(#x41 #x0f #x9c #xc1) (SETL R9L))
    "SETL R9L")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETL AL)) <int> <int>) -2 3))
    "Signed integer being less")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETL AL)) <int> <int>) 3 3))
    "Signed integer not being less")
(ok (equal? '(#x41 #x0f #x9d #xc1) (SETNL R9L))
    "SETNL R9L")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETNL AL)) <int> <int>) -2 3))
    "Signed integer not being greater or equal")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETNL AL)) <int> <int>) 3 3))
    "Signed integer being greater or equal")
(ok (equal? '(#x41 #x0f #x9e #xc1) (SETLE R9L))
    "SETLE R9L")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETLE AL)) <int> <int>) -2 -2))
    "Signed integer being less or equal")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETLE AL)) <int> <int>) 3 -2))
    "Signed integer not being less or equal")
(ok (equal? '(#x41 #x0f #x9f #xc1) (SETNLE R9L))
    "SETNLE R9L")
(ok (eqv? 0 ((asm ctx <byte> (list (CMP EDI ESI) (SETNLE AL)) <int> <int>) -2 -2))
    "Signed integer not being greater")
(ok (eqv? 1 ((asm ctx <byte> (list (CMP EDI ESI) (SETNLE AL)) <int> <int>) 3 -2))
...
(ok (equal? '(#x72 #x2a) (JB 42))
    "JB 42")
(ok (eqv? 3 ((asm ctx <int> (list (MOV EAX EDI) (CMP EAX 5) (JB 'l) (MOV EAX 5) 'l) <int>) 3))
    "Test JB with CF=1")
(ok (eqv? 5 ((asm ctx <int> (list (MOV EAX EDI) (CMP EAX 5) (JB 'l) (MOV EAX 5) 'l) <int>) 7))
    "Test JB with CF=0")
...
(ok (eqv? 42 ((asm ctx <int> (list (MOV (ptr <long> RSP -8) RBX)
                                   (SUB RSP 8)
                                   (MOV EBX 42)
                                   (MOV EAX EBX)
