...
(define (ldata) (begin
                  ...
                  (store (+ lptr 1) l2)
                  mem))
(define a (make <var> #:type <int> #:symbol 'a)); TODO: use skel <int>?
...
(define s (compose-from (sequence <int>) (list x y p)))
(define r (compose-from (sequence <int>) (list x y q)))
...
(ok (equal? (make <int> #:value a) (compose-from <int> (list a)))
    "Compose integer from variables")
(ok (equal? (make <intrgb> #:value (rgb a b c)) (compose-from <intrgb> (list a b c)))
    "Compose RGB value from variables")
(ok (let [(s (compose-from (sequence <byte>) (list a b c)))]
      (equal? (list a b c) (append (shape s) (strides s) (list (get-value s)))))
    "Compose sequence from variables")
(ok (let* [(m (compose-from (multiarray <byte> 2) (list a b c d e)))]
      (equal? (list c a d b e) (append (shape m) (strides m) (list (get-value m)))))
    "Compose 2D array from variables")
...
(ok (equal? (list a b c) (decompose (make <intrgb> #:value (rgb a b c))))
    "Decompose RGB value")
(ok (is-a? (get-value (skel <int>)) <var>)
...
(ok (equal? *p (compose-from (pointer <int>) (list p)))
