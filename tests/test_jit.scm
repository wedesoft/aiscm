...
(ok (equal? (MOV (ptr <byte> RAX) CL) (MOV (ptr <byte> RAX) ECX))
    "Size of target overrides size of source when specifying MOV instruction")
...
(ok (equal? (apply + (iota 10))
            ((wrap ctx <int> (list <int>)
                  (lambda (r x) (list (for [(i <long>) (MOV i 0) (CMP i x) (ADD i 1)] (ADD r i)) (RET)))) 10))
    "for loop")
