...
(ok (equal? (list (MOV (ptr <sint> RCX) SI))
            (let [(p (make <var> #:type <long> #:symbol 'p))
                  (x (make <var> #:type <sint> #:symbol 'x))]
              (subst (list (MOV (ptr <sint> p) x)) (list (cons p RCX) (cons x RSI)))))
    "Substitute pointer variable")
(let* [(a (make <var> #:type <long> #:symbol 'a))
       (b (make <var> #:type <long> #:symbol 'b))
       (c (make <var> #:type <long> #:symbol 'c))
       (s (lambda (cmd) (subst cmd (list (cons a RCX) (cons b RDX) (cons c RSI)))))]
  (ok (equal? (MOV RCX RDX) (s (MOV a b)))
      "Substitution works with 'MOV'")
  (ok (equal? (LEA RCX (ptr <byte> RDX)) (s (LEA a (ptr <byte> b))))
      "Substitution works with 'LEA")
  (ok (equal? (ADD RCX RDX) (s (ADD a b)))
      "Substitution works with 'ADD'")
  (ok (equal? (NEG RCX) (s (NEG a)))
      "Substitution works with 'NEG'")
  (ok (equal? (IMUL RCX RDX) (s (IMUL a b)))
      "Substitution works with 'IMUL'")
  (ok (equal? (IMUL RCX RDX 2) (s (IMUL a b 2)))
      "Substitution works with 'IMUL' and three arguments")
  (ok (equal? (CMP RCX RDX) (s (CMP a b)))
