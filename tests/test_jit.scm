...
(define e (make <var> #:type <int> #:symbol 'e))
...
(define s (make (sequence <int>) #:value p #:shape (list x) #:strides (list y)))
(define r (make (sequence <int>) #:value q #:shape (list x) #:strides (list y)))
...
(ok (equal?  (list a b) (variables (list (MOV a 0) (MOV (ptr <int> b) a))))
...
(ok (equal? (list <sint>) (map typecode (vars <sint>)))
    "Allocate variables to represent a short integer")
(ok (equal? (make-list 3 <byte>) (map typecode (vars <bytergb>)))
    "Allocate variables to represent a byte RGB value")
...
(ok (equal? (list (MOVZX CX AL) (RET))
            (register-allocate (append (code (to-type <sint> (parameter (skel <ubyte>)))) (list (RET)))))
...
(skip (equal? (list (MOV ECX EDI) (MOV EAX ECX) (RET))
            (assemble b (list a) (to-type <int> (parameter (make <int> #:value a)))))
    "Trivial type conversion")
(skip (equal? (list (MOVZX ECX DIL) (MOV EAX ECX) (RET))
            (assemble a (list u) (to-type <int> (parameter (make <ubyte> #:value u)))))
    "unsigned type conversion")
(skip (equal? (list (MOVSX ECX DIL) (MOV EAX ECX) (RET))
            (assemble a (list v) (to-type <int> (parameter (make <byte> #:value v)))))
    "Signed type conversion")
(skip (equal? (list (MOV CL DIL) (MOV AL CL) (RET))
            (assemble v (list a) (to-type <byte> (parameter (make <int> #:value a)))))
    "Typecast integer to byte")
(ok (equal? 42 ((jit ctx (list <int>) (cut to-type <int> <>)) 42))
    "Run trivial type conversion")
(ok (eq? <int> (type (class-of (+ (parameter (skel <int>)) (parameter (skel <int>))))))
    "Adding two integers returns an integer")
(ok (eq? <sint> (type (class-of (+ (parameter (skel <byte>)) (parameter (skel <sint>))))))
    "Adding a byte and a short integer returns a short integer")
(skip (equal? (list (MOV EAX EDI) (MOV EDX ESI) (MOV ECX EAX) (MOV EAX EDX) (ADD ECX EAX) (MOV EAX ECX) (RET))
            (assemble c (list a b) (+ (parameter (make <int> #:value a)) (parameter (make <int> #:value b)))))
    "Add two integers")
(skip (equal? (list (MOVSX EAX DIL) (MOV EDX ESI) (MOV ECX EAX) (MOV EAX EDX) (ADD ECX EAX) (MOV EAX ECX) (RET))
            (assemble c (list v b) (+ (parameter (make <byte> #:value v)) (parameter (make <int> #:value b)))))
    "Add byte and integer")
(skip (equal? (list (MOV EAX EDI) (MOVZX EDX SIL) (MOV ECX EAX) (MOV EAX EDX) (ADD ECX EAX) (MOV EAX ECX) (RET))
            (assemble c (list a u) (+ (parameter (make <int> #:value a)) (parameter (make <ubyte> #:value u)))))
...
(skip (equal? (list (MOV AX DI) (MOVSX DX SIL) (MOV CX AX) (MOV AX DX) (ADD CX AX) (MOV AX CX) (RET))
            (assemble o (list w v) (+ (parameter (make <sint> #:value w)) (parameter (make <byte> #:value v)))))
...
(skip (equal? (list (MOV (ptr <int> RDI) ESI) (RET))
            (assemble *p (list a) (parameter (make <int> #:value a))))
...
(ok (eq? <intrgb> (class-of (skel <intrgb>)))
    "Skeleton of integer RGB has correct type")
(ok (eq? <rgb> (class-of (get-value (skel <intrgb>))))
    "Skeleton of integer RGB has RGB value")
(ok (equal? (make-list 3 <int>) (map typecode (content (get-value (skel <intrgb>)))))
    "Variables in integer RGB skeleton have correct type")
(ok (equal? (list a) (decompose (make <int> #:value a)))
    "Decompose variable")
(ok (equal? (list p) (decompose *p))
    "Decompose pointer")
(ok (equal? (list a b c) (decompose (make <intrgb> #:value (rgb a b c))))
    "Decompose RGB value")
(ok (eq? <var> (class-of (get-value (skel <int>))))
    "'skel' creates a variable for an integer type")
(ok (equal? <int> (class-of (skel <int>)))
    "'skel' creates a value of correct integer type")
(ok (equal? <int> (typecode (get-value (skel <int>))))
    "'skel' creates a variable of correct integer type")
(ok (eq? (sequence <int>) (class-of (skel (sequence <int>))) )
    "'skel' can create sequences")
(ok (eq? (pointer <int>) (class-of (skel (pointer <int>))))
    "Skeleton of pointer has correct type")
(ok (eq? <long> (typecode (get-value (skel (pointer <int>)))))
...
(ok (equal? <long> (typecode (get-value (skel (sequence <byte>)))))
    "Pointer of sequence skeleton is a long integer")
(ok (equal? (list <long>) (map typecode (shape (skel (sequence <int>)))))
    "Shape of sequence is a list of long integers")
(ok (equal? (list <long>) (map typecode (strides (skel (sequence <int>)))))
    "Strides of sequence is a list of long integers")
(ok (equal? (make-list 3 <long>) (map typecode (decompose (skel (sequence <byte>)))))
    "Decomposing a sequence returns a list of 3 long integer variables")
(ok (equal? <long> (typecode (get-value (skel (multiarray <byte> 2)))))
    "Pointer of 2D array skeleton is a long integer")
(ok (equal? (list <long> <long>) (map typecode (shape (skel (multiarray <byte> 2)))))
    "Shape of 2D array skeleton is a list of long integers")
(ok (equal? (list <long> <long>) (map typecode (strides (skel (multiarray <byte> 2)))))
    "Strides of 2D array skeleton is a list of long integers")
(ok (equal? (make-list 5 <long>) (map typecode (decompose (skel (multiarray <byte> 2)))))
