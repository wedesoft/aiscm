...
(ok (equal? (list (MOV EAX EDI) (RET))
                  (build (list <int>) (lambda (a) (typecast <int> a))))
    "Build trivial type conversion with parametrisation")
(ok (equal? 42 ((translate ctx <int> (list <int>)
                           (lambda (r a) (list ((code (typecast <int> (parameter a))) r) (RET)))) 42))
    "Run trivial type conversion"); TODO: use jit
...
(ok (equal? (list (MOV EAX EDI) (MOV ECX ESI) (ADD EAX ECX) (RET))
            (build (list <int> <int>) +))
    "Parameterize code for adding two integers")
...
(ok (equal? 9 ((translate ctx <int> (list <int> <int> <int>)
                          (lambda (r a b c) (list ((code (+ (parameter a) (parameter b) (parameter c))) r)
                                                  (RET)))) 2 3 4)); TODO: use jit
    "Compiling a plus operation creates and equivalent machine program")
(ok (equal? (list (MOV DX CX) (MOVSX CX AL) (ADD DX CX))
            (subst ((code (+ (parameter w) (parameter v))) o)))
    "Adding a short integer and a byte does the required sign extension")
(ok (equal? 3 ((translate ctx <int> (list <int> <sint> <ubyte>)
                          (lambda (r a b c) (list ((code (+ (parameter a) (parameter b) (parameter c))) r)
                                                  (RET)))) 2 -3 4)); TODO: use jit
