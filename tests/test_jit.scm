...
(ok (eq? <rgb> (class-of (get-value (skeleton <intrgb>))))
    "Skeleton of integer RGB has RGB value")
(ok (equal? (make-list 3 <int>) (map typecode (content (get-value (skeleton <intrgb>)))))
...
(ok (equal? (make-list 3 <int>) (map typecode (decompose (skeleton <intrgb>))))
    "Decompose RGB value")
(ok (eq? <var> (class-of (get-value (skeleton <int>))))
...
(ok (equal? <int> (typecode (get-value (skeleton <int>))))
...
(ok (eq? <long> (typecode (get-value (skeleton (pointer <int>)))))
...
(ok (equal? <long> (typecode (get-value (skeleton (sequence <byte>)))))
...
(ok (equal? <long> (typecode (get-value (skeleton (multiarray <byte> 2)))))
