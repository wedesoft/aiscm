...
(ok (equal? (list (MOV ECX (ptr <int> RSP 8)) (MOV EAX ECX) (RET))
            (let [(args (map skel (make-list 7 <int>)))]
...
(ok (equal? (rgb a b c) (compose-from <intrgb> (list a b c)))
...
(ok (equal? (list (rgb 3 3 3)) (to-list ((jit ctx (list (sequence <ubytergb>) (sequence <ubytergb>)) -)
                                         (seq (rgb 6 5 4)) (seq (rgb 3 2 1)))))
    "subtract RGB sequences")
(ok (equal? (list (rgb 1 2 3)) (to-list ((jit ctx (list (sequence <ubytergb>) <ubyte>) -)
                                         (seq (rgb 4 5 6)) 3)))
    "subtract byte from byte RGB sequence")
(ok (equal? (list (rgb 3 2 1)) (to-list ((jit ctx (list (sequence <byte>) <ubytergb>) -)
                                         (seq <byte> 4) (rgb 1 2 3))))
    "subtract RGB value from byte sequence")
(ok (equal? (list (rgb 0 1 2)) (to-list ((jit ctx (list <ubytergb> (sequence <byte>)) -)
                                         (rgb 1 2 3) (seq <byte> 1))))
    "subtract byte sequence from RGB value")
