...
(ok (eqv? (+ i1 i2) ((asm ctx int (list (MOV EAX i2) (MOV RDX (idata)) (ADD EAX *RDX) (RET)))))
...
(ok (equal? '(#x48 #x8d #x51 #x04) (LEA RDX *RCX 4))
    "LEA RDX *RCX 4")
...
(ok (equal? '(#x48 #x8d #x04 #xb7) (LEA RAX *RDI RSI *4))
    "LEA RAX *RDI RSI *4")
(ok (eqv? i2 ((asm ctx int (list (MOV RCX (idata))
                                 (MOV RDI 1)
                                 (LEA RDX *RCX RDI *4)
...
(ok (equal? '(#x48 #x8d #x04 #x77) (LEA RAX *RDI RSI *2))
    "LEA RAX *RDI RSI *2")
(ok (eqv? i2 ((asm ctx int (list (MOV RCX (idata))
                                 (MOV RDI 2)
                                 (LEA RDX *RCX RDI *2)
...
(ok (equal? '(#x48 #x8d #x44 #x77 #x02) (LEA RAX *RDI RSI *2 2))
    "LEA RAX *RDI RSI *2 2")
(ok (eqv? i2 ((asm ctx int (list (MOV RCX (idata))
                                 (MOV RDI 3)
                                 (LEA RDX *RCX RDI *1 1)
