...
(ok (equal? (list (MOV RCX (ptr <long> RAX)))
            (subst (list (MOV p (ptr <long> RAX))) (list (cons p RCX))))
    "Substitute long integer variable with register")
(ok (equal? (list (MOV (ptr <int> RCX) ESI))
            (subst (list (MOV (ptr <int> p) a)) (list (cons p RCX) (cons a RSI))))
    "Substitute pointer variable")
(ok (equal? (MOV ECX EDX) (subst (MOV a b) (list (cons a RCX) (cons b RDX))))
    "Substitution works with 'MOV'")
(ok (equal? (MOVSX RCX EDX) (subst (MOVSX p b) (list (cons p RCX) (cons b RDX))))
    "Substitution works with 'MOVSX'")
(ok (equal? (MOVZX ECX DX) (subst (MOVZX a w) (list (cons a RCX) (cons w RDX))))
    "Substitution works with 'MOVZX'")
(ok (equal? (LEA RCX (ptr <byte> RDX)) (subst (LEA p (ptr <byte> q)) (list (cons p RCX) (cons q RDX))))
    "Substitution works with 'LEA")
(ok (equal? (SHL ECX) (subst (SHL a) (list (cons a RCX))))
    "Substitution works with 'SHL")
(ok (equal? (SHR ECX) (subst (SHR a) (list (cons a RCX))))
    "Substitution works with 'SHR")
(ok (equal? (SAL ECX) (subst (SAL a) (list (cons a RCX))))
    "Substitution works with 'SAL")
(ok (equal? (SAR ECX) (subst (SAR a) (list (cons a RCX))))
    "Substitution works with 'SAR")
(ok (equal? (ADD ECX EDX) (subst (ADD a b) (list (cons a RCX) (cons b RDX))))
    "Substitution works with 'ADD'")
(ok (equal? (PUSH ECX) (subst (PUSH a) (list (cons a RCX))))
    "Substitution works with 'PUSH'")
(ok (equal? (POP ECX) (subst (POP a) (list (cons a RCX))))
    "Substitution works with 'POP'")
(ok (equal? (NEG ECX) (subst (NEG a) (list (cons a RCX))))
    "Substitution works with 'NEG'")
(ok (equal? (SUB ECX EDX) (subst (SUB a b) (list (cons a RCX) (cons b RDX))))
    "Substitution works with 'SUB'")
(ok (equal? (IMUL ECX EDX) (subst (IMUL a b) (list (cons a RCX) (cons b RDX))))
    "Substitution works with 'IMUL'")
(ok (equal? (IMUL ECX EDX 2) (subst (IMUL a b 2) (list (cons a RCX) (cons b RDX))))
    "Substitution works with 'IMUL' and three arguments")
(ok (equal? (CMP ECX EDX) (subst (CMP a b) (list (cons a RCX) (cons b RDX))))
    "Substitution works with 'CMP'")
(ok (equal? (SETB CL) (subst (SETB u) (list (cons u RCX))))
...
(ok (equal? (list '() (list a) '()) (live (list 'x (MOV a 0) (RET))))
    "Live-analysis for definition of unused variable")
(ok (equal? (list (list a) (list a) (list b a) '()) (live (list (MOV a 0) (NOP) (MOV b a) (RET))))
...
(ok (equal? (list (MOV (ptr <long> RSP #x10) RBP) (MOV (ptr <long> RSP #x18) R12))
            (save-registers (list RBP R12) 2))
    "'save-registers' should generate instructions for saving registers on the stack")
(ok (equal? (list (MOV RBP (ptr <long> RSP #x10)) (MOV R12 (ptr <long> RSP #x18)))
            (load-registers (list RBP R12) 2))
    "'load-registers' should generate instructions for saving registers on the stack")
(ok (equal? (list (MOV R8D 0) (RET))
            (virtual-registers <int> (make-list 6 <int>)
                               (lambda x (list (MOV a 0) (RET))) #:registers (list R8)))
    "'virtual-registers' should use the specified set of registers")
(ok (equal? (list (MOV (ptr <long> RSP #x8) R12) (MOV R12D 0) (MOV R12 (ptr <long> RSP #x8)) (RET))
            (virtual-registers <int> (make-list 6 <int>)
                               (lambda x (list (MOV a 0) (RET))) #:registers (list R12)))
    "'virtual-registers' should save and restore callee-saved registers")
(ok (equal? (list (MOV (ptr <long> RSP #x10) R12)
                  (MOV R12D (ptr <int> RSP #x8))
                  (MOV R12 (ptr <long> RSP #x10))
                  (RET))
            (virtual-registers <int> (make-list 7 <int>)
                               (lambda x (list (RET))) #:registers (list R12)))
    "'virtual-registers' should take stack-offset into account")
