...
(ok (equal? '(#x48 #x8d #x04 #xb7) (LEA RAX (qword-ptr RDI RSI (scale 4))))
    "LEA RAX, [RDI + ESI * 4]")
(ok (eqv? i2 ((asm ctx int (list (MOV RCX (idata))
                                 (MOV RDI 1)
                                 (LEA RDX (qword-ptr RCX RDI (scale 4)))
                                 (MOV EAX (dword-ptr RDX))
                                 (RET)))))
    "Load integer from address in RCX with index times 4 using effective address")
(ok (equal? '(#x48 #x8d #x04 #x77) (LEA RAX (qword-ptr RDI RSI (scale 2))))
    "LEA RAX, [RDI + ESI * 2]")
(ok (eqv? i2 ((asm ctx int (list (MOV RCX (idata))
                                 (MOV RDI 2)
                                 (LEA RDX (qword-ptr RCX RDI (scale 2)))
                                 (MOV EAX (dword-ptr RDX))
                                 (RET)))))
    "Load integer from address in RCX with index times 2 using effective address")
(ok (equal? '(#x48 #x8d #x44 #x77 #x02) (LEA RAX (qword-ptr RDI RSI (scale 2) 2)))
    "LEA RAX, [RDI + ESI * 2 + 2]")
(ok (eqv? i2 ((asm ctx int (list (MOV RCX (idata))
                                 (MOV RDI 3)
                                 (LEA RDX (qword-ptr RCX RDI (scale 1) 1))
                                 (MOV EAX (dword-ptr RDX))
                                 (RET)))))
    "Load integer from address in RCX with index and offset using effective address")
(ok (eqv? i2 ((asm ctx int (list (MOV R9 (idata))
                                 (MOV R10 3)
                                 (LEA R11 (qword-ptr R9 R10 (scale 1) 1))
