...
(ok (equal? '(#x66 #x89 #xc3) (MOV BX AX))
    "MOV BX, AX")
(ok (equal? '(#x8b #x0a) (MOV ECX (addr RDX)))
    "MOV ECX, [RDX]")
(ok (equal? '(#x48 #x8b #x0a) (MOV RCX (addr RDX)))
    "MOV RCX, [RDX]")
(ok (equal? '(#x41 #x8b #x0b) (MOV ECX (addr R11)))
    "MOV ECX, [R11]")
(ok (equal? '(#x8b #x4a #x04) (MOV ECX (addr RDX 4)))
    "MOV ECX, [R11] + 4")
(ok (equal? '(#x8b #x4c #x24 #x04) (MOV ECX (addr RSP 4)))
    "MOV ECX, [RSP] + 4")
(ok (equal? '(#x45 #x8b #x53 #x04) (MOV R10D (addr R11 4)))
    "MOV R10D, [R11] + 4")
(ok (equal? '(#x4d #x8b #x53 #x04) (MOV R10 (addr R11 4)))
    "MOV R10, [R11] + 4")
(ok (equal? '(#x8a #x0a) (MOV CL (addr RDX)))
    "MOV CL, [RDX]")
(ok (equal? '(#x66 #x8b #x0a) (MOV CX (addr RDX)))
    "MOV CX, [RDX]")
(ok (equal? '(#x89 #x11) (MOV (addr RCX) EDX))
    "MOV [RCX], EDX")
(ok (equal? '(#x44 #x89 #x01) (MOV (addr RCX) R8D))
    "MOV [RCX], R8D")
(ok (equal? '(#xc3) (RET))
    "RET # near return")
...
(ok (equal? '(#x45 #x01 #xfe) (ADD R14D R15D))
    "ADD R14D, R15D")
(ok (equal? '(#x66 #x01 #xd1) (ADD CX DX))
    "ADD CX, DX")
(ok (equal? '(#x00 #xd1) (ADD CL DL))
    "ADD CL, DL")
(ok (equal? '(#x03 #x0a) (ADD ECX (addr RDX)))
    "ADD ECX, [RDX]")
...
(ok (eqv? (+ i1 i2) ((asm ctx int (list (MOV EAX i2) (MOV RDX (idata)) (ADD EAX (addr RDX)) (RET)))))
...
(ok (eqv? l1 ((asm ctx long (list (MOV RDX l1) (PUSH RDX) (POP RAX) (RET)))))
    "Use PUSH and POP")
(ok (eqv? i1 ((asm ctx int (list (MOV RCX (idata)) (MOV EAX (addr RCX)) (RET)))))
    "Load integer from address in RCX")
(ok (eqv? i1 ((asm ctx int (list (MOV R10 (idata)) (MOV EAX (addr R10)) (RET)))))
    "Load integer from address in R10")
(ok (eqv? i2 ((asm ctx int (list (MOV RCX (idata)) (MOV EAX (addr RCX 4)) (RET)))))
    "Load integer from address in RCX with offset")
(ok (eqv? i2 ((asm ctx int (list (MOV R9 (idata)) (MOV EAX (addr R9 4)) (RET)))))
    "Load integer from address in R9D with offset")
(ok (eqv? l1 ((asm ctx long (list (MOV RCX (ldata)) (MOV RAX (addr RCX)) (RET)))))
    "Load long integer from address in RCX")
(ok (eqv? l2 ((asm ctx long (list (MOV RCX (ldata)) (MOV RAX (addr RCX 8)) (RET)))))
    "Load long integer from address in RCX with offset")
(ok (eqv? w1 ((asm ctx short (list (MOV RCX (wdata)) (MOV AX (addr RCX)) (RET)))))
    "Load short integer from address in RCX")
(ok (eqv? w2 ((asm ctx short (list (MOV RCX (wdata)) (MOV AX (addr RCX 2)) (RET)))))
    "Load short integer from address in RCX with offset")
(ok (eqv? b1 ((asm ctx int8 (list (MOV RCX (bdata)) (MOV AL (addr RCX)) (RET)))))
    "Load byte from address in RCX")
(ok (eqv? b2 ((asm ctx int8 (list (MOV RCX (bdata)) (MOV AL (addr RCX 1)) (RET)))))
    "Load byte from address in RCX with offset")
(ok (equal? '(#x48 #x8d #x51 #x04) (LEA RDX (addr RCX 4)))
    "LEA RDX, [RCX + 4]")
(ok (equal? '(#x48 #x8d #x44 #x24 #xfc) (LEA RAX (addr RSP -4)))
    "LEA RAX, [RSP - 4]")
...
(ok (eqv? 17 ((asm ctx int (list (MOV EAX (addr RSP #x8)) (RET))
                   int int int int int int int int) 2 3 5 7 11 13 17 19))
    "Function return seventh integer argument")
(ok (eqv? 19 ((asm ctx int (list (MOV EAX (addr RSP #x10)) (RET))
                   int int int int int int int int) 2 3 5 7 11 13 17 19))
