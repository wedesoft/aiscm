...
(ok (equal? a (get-value (get-value (fragment a))))
    "The value of a trivial expression is itself")
(ok (equal? <int> (typecode (get-value (+ (fragment a) (fragment b)))))
...
(ok (equal? <sint> (typecode (get-value (typecast <sint> (fragment u)))))
...
(ok (equal? <sint> (typecode (get-value (+ (fragment u) (fragment w)))))
    "Adding a short integer and a byte returns a byte")
(ok (equal? (list (MOVSX CX DIL) (MOV DX SI) (MOV AX CX) (ADD AX DX) (RET))
