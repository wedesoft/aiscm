...
(ok (equal? '(#x8b #x4a #x04) (MOV ECX *RDX 4))
    "Read data from address with byte offset")
(ok (equal? '(#x8b #x4c #x24 #x04) (MOV ECX *RSP 4))
    "Read data from stack address with byte offset")
(ok (equal? '(#x89 #x11) (MOV *RCX EDX))
    "Write data from register EDX to address")
(ok (equal? '(#x44 #x89 #x01) (MOV *RCX R8W))
    "Write data from register R8W to address")
(ok (equal? '(#xc3) (RET))
    "RET # near return")
(ok (eqv? 42 ((asm ctx int (list (MOV EAX 42) (RET)))))
    "Function returning constant in EAX")
(ok (eqv? (ash 42 32) ((asm ctx long (list (MOV RAX (ash 42 32)) (RET)))))
    "Function returning constant in RAX")
(ok (eqv? 42 ((asm ctx int (list (MOV ECX 42) (MOV EAX ECX) (RET)))))
    "Function copying content from ECX")
(ok (eqv? 42 ((asm ctx int (list (MOV R14W 42) (MOV EAX R14W) (RET)))))
...
(ok (equal? '(#xd1 #xe5) (SAL EBP))
    "SAL EBP, 1")
(ok (eqv? 84 ((asm ctx int (list (MOV EAX 42) (SHL EAX) (RET)))))
...
(ok (equal? '(#xd1 #xfd) (SAR EBP))
    "SAR EBP, 1")
(ok (eqv? 21 ((asm ctx int (list (MOV EAX 42) (SHR EAX) (RET)))))
...
(ok (equal? '(#x90) (NOP))
    "NOP # no operation")
(ok (eqv? 42 ((asm ctx int (list (MOV EAX 42) (NOP) (NOP) (RET)))))
...
(ok (equal? '(#x5f) (POP EDI))
    "POP EDI")
(ok (eqv? 42 ((asm ctx int (list (MOV EDX 42) (PUSH EDX) (POP EAX) (RET)))))
