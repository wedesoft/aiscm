...
(ok (equal? '(label tst) (label tst))
    "Create label")
(ok (not (throws? (asm ctx int (list (MOV EAX i1) (label tst) (RET)))))
    "Assembler should tolerate labels")
(ok (eqv? 0 (assq-ref (labels (list (label test))) 'test))
    "Sole label maps to zero")
(ok (eqv? 0 (assq-ref (labels (list (label test) (NOP))) 'test))
    "Label at beginning of code is zero")
(ok (eqv? (length (MOV EAX ESI)) (assq-ref (labels (list (MOV EAX ESI) (label test))) 'test))
    "Label after MOV EAX ESI statement maps to length of that statement")
(ok (equal? '(1 2) (let ((a (labels (list (NOP) (label x) (NOP) (label y)))))
