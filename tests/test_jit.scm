...
(ok (eq? <int> (type (parameter (skel (pointer <int>)))))
...
(ok (equal? (list <int>) (map typecode (decompose (skel <int>))))
    "Decompose integer")
(ok (equal? (list <long>) (map typecode (decompose (skel (pointer <int>)))))
    "Decompose pointer")
(ok (equal? (make-list 3 <int>) (map typecode (decompose (skel <intrgb>))))
    "Decompose RGB value")
...
(ok (equal? <int> (type (project (parameter (skel (sequence <int>))))))
