...
(ok (equal? (list (MOV EAX EDI) (ADD EAX ESI))
            (let [(expr (+ (fragment a) (fragment b)))]
              (substitute-variables (get-code expr)
                                    (list (cons a RDI) (cons b RSI) (cons (get-value (get-value expr)) RAX)))))
    "Applying + to code fragments creates a program adding numbers")
(ok (equal? <int> (typecode (get-value (+ (fragment a) (fragment b)))))
    "The type of the sum of two values is coerced properly")
(ok (equal? (list (MOV EAX EDI) (ADD EAX ESI) (RET))
            (build (list a b) (+ (fragment a) (fragment b))))
    "Compiling the fragment creates correct assembler code")
