...
(let [(out (expression (skeleton (sequence <int>))))]
  (ok (eq? (iterator (term out)) (iterator out))
      "retrieve iterator pointer from tensor expression")
  (ok (eq? (step (term out)) (step out))
      "retrieve step variable from tensor expression")
...
(let* [(a    (expression (skeleton (sequence <int>))))
       (b    (skeleton <int>))
       (f    (+ a b))
       (out  (expression (skeleton (sequence <int>))))
       (incr (step a))
       (p    (iterator out))
       (q    (iterator a))]
  ...
  (ok (equal? (list (ADD q incr)) (increment f))
      "loop should increment input array iterator")
  (ok (equal? q (value (car (arguments (body f)))))
...
(let* [(a    (skeleton <int>))
       (b    (expression (skeleton (sequence <int>))))
       (f    (+ a b))
       (out  (expression (skeleton (sequence <int>))))
       (incr (step b))
       (p    (iterator out))
  ...
  (ok (equal? q (value (cadr (arguments (body f)))))
