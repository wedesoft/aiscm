...
(ok (not (throws? (asm ctx int (list (MOV EAX i1) (label tst) (RET)))))
    "Assembler should tolerate labels")
(ok (eqv? 0 (assq-ref (label-offsets (list (label tst))) 'tst))
    "Sole label maps to zero")
(ok (eqv? 0 (assq-ref (label-offsets (list (label tst) (NOP))) 'tst))
    "Label at beginning of code is zero")
(ok (eqv? (length (MOV EAX ESI))
          (assq-ref (label-offsets (list (MOV EAX ESI) (label tst))) 'tst))
    "Label after MOV EAX ESI statement maps to length of that statement")
(ok (equal? '(1 2) (let ((a (label-offsets (list (NOP) (label x) (NOP) (label y)))))
                     (map (cut assq-ref a <>) '(x y))))
    "Map multiple labels")
(ok (eq? 'tst (get-target (JMP tst)))
    "Target of JMP to label")
(ok (eqv? 5 (assq-ref (label-offsets (list (JMP tst) (label tst))) 'tst))
    "Label after JMP statement maps to 5")
(ok (equal? '(#xe9 #x05 #x00 #x00 #x00) (resolve (JMP tst) 0 '((tst . 5))))
    "Resolve jump address with zero offset")
(ok (equal? '(#xe9 #x03 #x00 #x00 #x00) (resolve (JMP tst) 2 '((tst . 5))))
    "Resolve jump address with offset 2")
(ok (equal? (list '(#xe9 #x02 #x00 #x00 #x00)) (resolve-jumps (list (JMP tst)) '((tst . 7))))
    "Resolve jump address in trivial program")
(ok (equal? (list '(#xe9 #x02 #x00 #x00 #x00) (NOP)) (resolve-jumps (list (JMP tst) (NOP)) '((tst . 7))))
    "Resolve jump address in program with trailing NOP")
(ok (equal? (list (NOP) '(#xe9 #x01 #x00 #x00 #x00)) (resolve-jumps (list (NOP) (JMP tst)) '((tst . 7))))
    "Resolve jump address in program with leading NOP")
(ok (equal? (list (NOP) (NOP)) (resolve-jumps (list (NOP) (label tst) (NOP)) '()))
