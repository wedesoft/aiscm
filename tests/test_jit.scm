...
(store p (make <int> #:value 42))
(store (+ p 1) (make <int> #:value 13))
...
(ok (eqv? 42 (jit-call (list (MOV ECX (pointer-address (get-memory m)))
                             (MOV EAX *ECX)
                             (RET))))
    "Function loading value address given as integer")
(ok (eqv? 42 (jit-call (list (MOV ECX m)
                             (MOV EAX *ECX)
                             (RET))))
    "Function loading value from address given as pointer")
(ok (eqv? 13 (jit-call (list (MOV ECX m)
                             (ADD ECX 4)
                             (MOV EAX *ECX)
                             (RET))))
...
(ok (eqv? 21 (begin (jit-call (list (MOV ESI (+ m 8))
                                    (MOV EBX 21)
                                    (MOV *ESI EBX)
                                    (RET)))
                    (get-value (fetch (+ p 2)))))
