...
(skip (equal? (list (MOV (ptr <byte> RDI) 42) (RET))
...
(skip (equal? (list (MOV ECX 42) (MOV EAX ECX) (RET))
...
(skip (equal? (list (MOV ECX (ptr <int> RSP 8)) (MOV EAX ECX) (RET))
...
(skip (eqv? 3 ((translate ctx <int> '()  (lambda (r)
...
(skip (equal? (list (MOV (ptr <int> RSP -8) ESI) (MOV ESI EDI) (MOV EDI (ptr <int> RSP -8)) (ADD ESI EDI) (RET))
            (pass-parameter-variables <null> (list <int> <int>) (lambda (a b) (list (MOV c a) (ADD c b) (RET)))
                                      #:registers (list RSI RDI)))
    "Spill register-parameter to the stack")
(skip (equal? (list (ADD EDI 1) (MOV EDI (ptr <int> RSP 8)) (ADD EDI 2) (MOV (ptr <int> RSP 8) EDI) (RET))
...
(ok (equal? (list (CMP EAX 0) (JE 7) (SUB EAX 1) (JMP -14))
    ...
    "'until' loop")
(skip (equal? (apply + (iota 10))
            ((translate ctx <int> '()
                  (lambda (r) (list (for [(i <int>) (MOV i 0) (CMP i 10) (INC i)] (ADD r i)) (RET))))))
    "'for' loop")
(skip (equal? 5 ((translate ctx <int> (list <int>)
...
(skip (not (equal? (list (MOV CL AL) (SHL ECX CL) (RET))
            (register-allocate (list (MOV CL u) (SHL a CL) (RET))
                               #:predefined (list (cons a RCX))
                               #:blocked (list (cons RCX '(0 . 1))))))
    "Override predefined registers if they are blocked")
