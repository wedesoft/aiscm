...
(ok (equal? '(#x41 #x89 #x04 #x24) (MOV (ptr <int> R12) EAX))
    "MOV [R12], EAX")
(ok (equal? (MOV EDX ECX) (MOV EDX RCX))
    "MOV takes size of operation from first argument in 32-bit case")
(ok (equal? (MOV DX CX) (MOV DX RCX))
    "MOV takes size of operation from first argument in 16-bit case")
(ok (equal? (MOV DX CX) (MOV DX ECX))
    "MOV takes size of operation from first argument in 16-bit case")
(ok (equal? (MOV DL CL) (MOV DL RCX))
    "MOV takes size of operation from first argument in 8-bit case")
(ok (equal? (MOV DL CL) (MOV DL ECX))
    "MOV takes size of operation from first argument in 8-bit case")
(ok (equal? (MOV DL CL) (MOV DL CX))
    "MOV takes size of operation from first argument in 8-bit case")
(ok (equal? '(#xc3) (RET))
    "RET # near return")
