...
(ok (eqv? (- (get-value i1)) (get-value (- i1)))
    "negate integer")
(ok (equal? '(3 3 3) (sequence->list (fill <int> 3 3)))
    "fill integer sequence")
(ok (equal? '(3 3 3) (sequence->list (fill <sint> 3 3)))
    "fill short integer sequence")
(ok (equal? '(3 3 3) (sequence->list (fill <byte> 3 3)))
    "fill byte sequence")
(ok (equal? '(3 3 3) (sequence->list (fill <long> 3 3)))
...
(ok (equal? '(1 -2 -3) (sequence->list (- (list->sequence '(-1 2 3)))))
    "negate sequence")
(ok (equal? '(4 4 4) (sequence->list (+ (fill <int> 3 3) 1)))
    "add integer to integer sequence")
(ok (equal? '(257 257 257) (sequence->list (+ (fill <byte> 3 1) 256)))
    "add integer to byte sequence")
(ok (equal? '(257 257 257) (sequence->list (+ 256 (fill <byte> 3 1))))
    "add byte sequence to integer")
(ok (equal? '(4 4 4) (sequence->list (+ 1 (fill <int> 3 3))))
    "add integer sequence to integer")
(ok (equal? '(4 4 4) (sequence->list (+ (fill <int> 3 1) (fill <int> 3 3))))
    "add integer sequences")
(ok (equal? '(4 4 4) (sequence->list (+ (fill <byte> 3 1) (fill <sint> 3 3))))
    "add byte and short integer sequences")
(ok (equal? '(4 4 4) (sequence->list (+ (fill <sint> 3 1) (fill <byte> 3 3))))
    "add short integer and byte sequences")
(ok (equal? '(255 254 253) (sequence->list (+ (list->sequence '(-1 -2 -3)) 256)))
    "sign-expand negative values when adding byte sequence and short integer")
(ok (equal? '(255 254 253) (sequence->list (+ 256 (list->sequence '(-1 -2 -3)))))
    "sign-expand negative values when adding short integer and byte sequence")
(ok (equal? '(255) (sequence->list (+ (list->sequence '(-1)) (list->sequence '(256)))))
    "sign-expand negative values when adding byte sequence and short integer sequence")
(ok (equal? '(255) (sequence->list (+ (list->sequence '(256)) (list->sequence '(-1)))))
