...
(ok (equal? '(253 252 250) (multiarray->list (bitwise-not (list->multiarray '(2 3 5)))))
    "bitwise negation of array")
(ok (equal? '(#f #t #f) (multiarray->list (is-zero? (list->multiarray '(-1 0 1)))))
    "compare bytes with zero")
(ok (equal? '(#f #t #f) (multiarray->list (is-zero? (to-type (list->multiarray '(-1 0 1)) <int>))))
    "compare integers with zero")
(ok (equal? '(#t #f #t) (multiarray->list (is-nonzero? (list->multiarray '(-1 0 1)))))
    "check whether bytes are not zero")
(ok (equal? '(#f #t #f) (multiarray->list (do-not (list->multiarray '(#t #f #t)))))
