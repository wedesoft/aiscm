...
(ok (equal? '(3 5) (multiarray->list (+ (downsample (list->multiarray '(1 2 3 4)) 2) 1)))
    "add 1 to downsampled array")
(ok (equal? '(3 5) (multiarray->list (+ 1 (downsample (list->multiarray '(1 2 3 4)) 2))))
    "add downsampled array to 1")
(ok (equal? '(4 8) (let [(s (downsample (list->multiarray '(1 2 3 4)) 2))] (multiarray->list (+ s s))))
