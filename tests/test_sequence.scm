...
(ok (equal? <ubyte> (typecode (list->array '(1 2 3))))
    "Typecode of converted list of unsigned bytes")
(ok (equal? <byte> (typecode (list->array '(1 -1))))
    "Typecode of converted list of signed bytes")
(ok (eqv? 3 (size (list->array '(1 2 3))))
    "Size of converted list")
(ok (equal? '(2 3 5) (begin (set s3 '(2 3 5)) (array->list s3)))
    "Assignment list to sequence")
(ok (equal? '(3 3 3) (begin (set s3 3) (array->list s3)))
    "Assignment number to sequence")
(ok (equal? '(2 3 5) (set s3 '(2 3 5)))
    "Return value of assignment to sequence")
(ok (equal? '(2 4 8) (array->list (list->array '(2 4 8))))
...
(ok (equal? '(1 2 3) (array->list (get (list->array '(1 2 3)))))
