...
(ok (equal? (list <bool>) (types <bool>))
    "'types' returns the type itself")
(ok (equal? '(#t) (map get-value (content (make <bool> #:value #t))))
    "'content' returns the value")
  (ok (eqv? #t (get-value (construct <bool> (list (make <bool> #:value #t)))))
    "'construct' passes booleans through")
