...
(ok (eqv? 63 ((jit ctx (list <int>) (lambda (x) (+ x (native-call (native-method <int> (list <int>) jit-constant-fun))))) 21))
    "Compile function call and plus operation to test that caller-saved registers get blocked")
(ok (eqv? 2 ((jit ctx (list <int> <int>) (lambda (x y) (native-call (native-method <int> (list <int> <int>) jit-subtracting-fun) y x))) 5 7))
    "Compile function call taking two arguments after swapping them")
(ok (eqv? 5 ((jit ctx (make-list 3 <int>) (lambda (x y z) (native-call (native-method <int> (list <int> <int>) jit-subtracting-fun) x (+ y z)))) 10 2 3))
    "Pass result of expression to function call")
(ok (eqv? 42 ((jit ctx (list <int> <int>) (lambda (a b) (native-call (native-method <int> (make-list 7 <int>) jit-seven-arguments) a a a a a a b))) 123 42))
