...
(ok (let [(t (clock))] (synchronise #t t 1 (compose sleep inexact->exact round)) (>= (elapsed t) 1))
    "Wait for clock to synchronise to given value")
(ok (let [(t (clock))] (sleep 1) (synchronise #t t 1 (compose sleep inexact->exact round)) (< (elapsed t) 2))
    "No waiting required if clock is already synchronised to given value")
(ok (not (throws? (let [(t (clock))] (synchronise #t t -1 (compose sleep inexact->exact round)))))
    "Do not attempt to wait negative time")
(ok (let [(t (clock))] (eqv? 42 (synchronise 42 t 0 identity)))
