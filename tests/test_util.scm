...
(ok (let [(t (clock))] (synchronise t 1 (compose sleep inexact->exact round) #t) (>= (elapsed t) 1))
    "Wait for clock to synchronise to given value")
(ok (let [(t (clock))] (sleep 1) (synchronise t 1 (compose sleep inexact->exact round) #t) (< (elapsed t) 2))
    "No waiting required if clock is already synchronised to given value")
(ok (not (throws? (let [(t (clock))] (synchronise t -1 (compose sleep inexact->exact round) #t))))
    "Do not attempt to wait negative time")
(ok (let [(t (clock))] (eqv? 42 (synchronise t 0 identity 42)))
