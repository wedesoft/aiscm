...
(ok (null? (parameters-to-spill '(a) (list (cons 'a RAX))))
    "a parameter does not need spilling if a register was allocated for it")
(ok (null? (parameters-to-spill '(a) '()))
...
(ok (null? (parameters-to-fetch '(g) '()))
    "stack parameter without allocated register does not need fetching")
(ok (null? (parameters-to-fetch '(g) (list (cons 'g (ptr <int> RSP -8)))))
...
(let [(a (var <int>))
  ...
  (ok (equal? (list (MOV EDX (ptr <int> RSP 16))) (replace-variables (list (cons a (ptr <int> RSP 16))) (MOV EDX a) RAX))
      "read input variable from spill location")
  (ok (equal? (list (MOV AX (ptr <sint> RSP 16)) (CMP AX 0))
              (replace-variables (list (cons x (ptr <sint> RSP 16))) (CMP x 0) RAX))
...
(ok (lset= eq? (list RBP R12) (select-callee-saved (list RAX RBP R10 R10 R12 R12)))
    "'select-callee-saved' should extract the set of callee-saved registers")
...
(let [(a (var <int>))]
  (ok (null? ((spill-parameters (list a)) (list (cons a RDI))))
  ...
  (ok (null? ((fetch-parameters (list a)) (list (cons a (ptr <int> RSP +8)))))
