...
(ok (equal? 127 (get-value (unpack <byte> (pack (make-byte 127)))))
  "pack and unpack signed byte")
(ok (equal? -128 (get-value (unpack <byte> (pack (make-byte -128)))))
  "pack and unpack signed byte with negative number")
(ok (equal? 32767 (get-value (unpack <sint> (pack (make-sint 32767)))))
  "pack and unpack signed short integer")
(ok (equal? -32768 (get-value (unpack <sint> (pack (make-sint -32768)))))
  "pack and unpack signed short integer with negative number")
(ok (equal? 2147483647 (get-value (unpack <int> (pack (make-int 2147483647)))))
  "pack and unpack signed integer")
(ok (equal? -2147483648 (get-value (unpack <int> (pack (make-int -2147483648)))))
