...
(let [(p (skeleton (pointer <sintrgb>)))
      (c (skeleton <sintrgb>))]
  (ok (equal? (MOV (ptr <sint> (get p)  ) (get (red   c))) (car   (code p c)))
      "Writing RGB to memory copies red channel")
  (ok (equal? (MOV (ptr <sint> (get p) 2) (get (green c))) (cadr  (code p c)))
      "Writing RGB to memory copies green channel")
  (ok (equal? (MOV (ptr <sint> (get p) 4) (get (blue  c))) (caddr (code p c)))
      "Writing RGB to memory copies blue channel")
  (ok (equal? (MOV (get (red   c))(ptr <sint> (get p)  )) (car   (code c p)))
      "Reading RGB from memory copies red channel")
  (ok (equal? (MOV (get (green c))(ptr <sint> (get p) 2)) (cadr  (code c p)))
      "Reading RGB from memory copies green channel")
  (ok (equal? (MOV (get (blue  c))(ptr <sint> (get p) 4)) (caddr (code c p)))
