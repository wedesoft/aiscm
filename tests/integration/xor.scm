...
(run s '() (tf-assign m1 (tf-truncated-normal (arr <int> 2 2) #:dtype <float>)))
(define b1 (tf-variable #:dtype <float> #:shape '(2)))
(run s '() (tf-assign b1 (arr <float> 0 0)))
(define h1 (tf-tanh (tf-add (tf-mat-mul x m1) b1)))

(define m2 (tf-variable #:dtype <float> #:shape '(2 1)))
(run s '() (tf-assign m2 (tf-truncated-normal (arr <int> 2 1) #:dtype <float>)))
