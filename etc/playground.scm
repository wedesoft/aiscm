...
(define (replace-variables allocation cmd temporaries)
  ...
  (let [(substituted (substitute-variables cmd allocation))]
    (if (is-a? substituted <cmd>)
      (let* [(target    (car (append (output cmd) (input cmd))))
             (location  (assq-ref allocation target))
             (is-input  (memv target (input cmd)))
             (is-output (memv target (output cmd)))]
        (filter identity
          (list (and is-input  (MOV (to-type (typecode target) (car temporaries)) location))
                (substitute-variables cmd (list (cons target (car temporaries))))
                (and is-output (MOV location (to-type (typecode target) (car temporaries)))))))
