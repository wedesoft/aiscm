...
AC_ARG_WITH([llvm],
    ...
    if test "$withval" = "no"; then
        ...
        want_llvm="yes"
        ac_llvm_config_path=`which llvm-config-17`
    else
	    want_llvm="yes"
	...
	if test "x$want_llvm" = "xyes"; then
      ...
      LIBS="$LIBS_SAVED"
		...
		if test "$succeeded" != "yes" ; then
			AC_MSG_ERROR([[We could not detect the llvm libraries make sure that llvm-config-17 is on your path or specified by --with-llvm.]])
		else
