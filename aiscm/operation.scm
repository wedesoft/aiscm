...
(define-method (duplicate (a <indexer>) (b <convolution>))
  ...
  (if (null? (shape kernel))
    ...
    (let [(iterator (var <long>))
          ...
          (k0       (var <long>))
          (kstep    (var <long>)) ]
      (append (list (MOV offset (value (dimension kernel)))
                    (SHR offset)
                    (IMUL offset (value (stride data)))
                    (IMUL offset offset (size-of (typecode data)))
                    (MOV a0 (value a))
                    (IMUL astep (value (stride a)) (size-of (typecode a)))
                    (MOV d0 (value data))
