...
(define-method (fill (t <meta<element>>) (n <integer>) value)
  (let* [(dx    (reg t RDX))
         (code  (asm ctx void (list (LEA RCX (ptr t RDI RSI (scale t)))
                                    (CMP RDI RCX)
                                    (JE 'ret)
                                    'loop
                                    (MOV (ptr t RDI) dx)
...
(define-method (+ (a <sequence<>>) (b <element>))
  (let* [(ta    (typecode (class-of a)))
         ...
         (axr   (reg tr RAX))
         (dx    (reg tr RDX))
         (code  (asm ctx void (list (LEA R8 (ptr tr RDI RCX (scale tr)))
                                    (CMP RDI R8)
                                    (JE 'ret)
                                    'loop
                                    (mova axr (ptr ta RSI))
...
(define-method (- (a <sequence<>>))
  (let* [(t     (typecode (class-of a)))
         (ax    (reg t RAX))
         (code  (asm ctx void (list (LEA RCX (ptr t RDI RDX (scale t)))
                                    (CMP RDI RCX)
                                    (JE 'ret)
                                    'loop
                                    (MOV ax (ptr t RSI))
