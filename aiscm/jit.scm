...
(define hex '(#x0 #x1 #x2 #x3 #x4 #x5 #x6 #x7 #x8 #x9 #xa #xb #xc #xd #xe #xf))
...
(define reg8-list (map (cut make <reg<8>> #:code <>) hex))
(define-method (reg (type <meta<reg<8>>>) (code <integer>)) (list-ref reg8-list code))
(for-each (lambda (sym val) (toplevel-define! sym (reg <reg<8>> val)))
...
(define reg16-list (map (cut make <reg<16>> #:code <>) hex))
(define-method (reg (type <meta<reg<16>>>) (code <integer>)) (list-ref reg16-list code))
(for-each (lambda (sym val) (toplevel-define! sym (reg <reg<16>> val)))
...
(define reg32-list (map (cut make <reg<32>> #:code <>) hex))
(define-method (reg (type <meta<reg<32>>>) (code <integer>)) (list-ref reg32-list code))
(for-each (lambda (sym val) (toplevel-define! sym (reg <reg<32>> val)))
...
(define reg64-list (map (cut make <reg<64>> #:code <>) hex))
(define-method (reg (type <meta<reg<64>>>) (code <integer>)) (list-ref reg64-list code))
(for-each (lambda (sym val) (toplevel-define! sym (reg <reg<64>> val)))
