...
(define-syntax-rule (unary-mutating-fun name cmd)
...
(define-syntax-rule (unary-functional-fun name cmd); TODO: refactor
  (define-method (name (a <parameter>))
    (make <function> #:arguments (list a)
                     #:type (to-type <bool> (type a))
                     #:project (lambda () (name (body a)))
                     #:term (lambda (out) (unary-functional-cmd cmd (term out) (term a))))))
(define-syntax-rule (unary-mutating-asm name cmd)
  (begin (mutating-op cmd)
         (unary-mutating-fun name cmd)))
...
(define-syntax-rule (binary-mutating-fun name cmd)
...
(define-syntax-rule (binary-mutating-asm name cmd)
  (begin (mutating-op cmd)
         (binary-mutating-fun name cmd)))
...
(define-unary-op unary-mutating-asm    -  NEG)
(define-unary-op unary-mutating-asm    ~  NOT)
(define-unary-op unary-functional-fun  =0 test-zero)
...
(define-binary-op binary-mutating-asm +  ADD)
(define-binary-op binary-mutating-asm -  SUB)
(define-binary-op binary-mutating-asm *  IMUL)
(define-binary-op binary-mutating-fun << shl)
(define-binary-op binary-mutating-fun >> shr)
(define-binary-op binary-mutating-asm &  AND)
(define-binary-op binary-mutating-asm |  OR)
(define-binary-op binary-mutating-asm ^  XOR)
