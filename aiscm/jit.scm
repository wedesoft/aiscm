...
(define-syntax-rule (unary-fun name conversion kind cmd)
  (define-method (name (a <param>)) (make-function name conversion kind cmd a)))
...
(define-syntax-rule (binary-fun name conversion kind cmd)
  (define-method (name (a <param>) (b <param>)) (make-function name conversion kind cmd a b)))
(define-syntax-rule (binary-asm name conversion kind cmd)
  (begin (mutating-op cmd)
         (binary-fun name conversion kind cmd)))
...
(define-syntax-rule (define-binary-delegate name delegate)
 (define-method (name (a <element>) (b <element>))
   (let [(f (jit ctx (map class-of (list a b)) delegate))]
     ...
     (name a b))))
(define-syntax-rule (define-binary-op define-op conversion kind name cmd)
  (begin (define-op name conversion kind cmd)
