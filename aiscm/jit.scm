...
(define (unary-mutating op out a)
  (insert-intermediate a out (lambda (tmp)
    (list (op (get (term tmp)))))))
(define (unary-extract op out a)
  (list (code (term out) (op (term a)))))
(define (binary-mutating op out a b)
  (let [(t (reduce coerce #f (map type (list a b))))]
    (insert-intermediate a out (lambda (tmp-a)
      (ensure-intermediate t b (lambda (tmp-b)
        (list (op (operand tmp-a) (operand tmp-b)))))))))
(define (functional-code op out . args)
  (let* [(typecode      (reduce coerce #f (map type args)))
         (mask          (map (requires-intermediate? typecode) args))
