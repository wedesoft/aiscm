(define-module (aiscm jit)
  ...
  #:export (<jit-context> <reg<>> <reg<8>> <reg<16>> <reg<32>> <reg<64>> <jmpcc>
  ...
  (define (iterate cmd acc)
    (let ((offsets (car acc))
      ...
      (if (is-a? cmd <symbol>)
        ...
        (let ((len-cmd (if (is-a? cmd <jmpcc>) (len cmd) (length cmd))))
          (cons offsets (+ offset len-cmd))))))
  (car (fold iterate (cons '() 0) commands)))
(define-class <jmpcc> ()
  (target #:init-keyword #:target #:getter get-target)
  (opcode #:init-keyword #:opcode #:getter get-opcode))
(define-method (len (self <jmpcc>)) 2)
(define-method (JMPcc (target <symbol>) (code <integer>))
  (make <jmpcc> #:target target #:opcode code))
(define-method (JMPcc (target <integer>) (code <integer>))
  (append (opcode code) (raw target 8)))
(define-method (resolve (self <jmpcc>) (offset <integer>) offsets)
  (JMPcc (- (assq-ref offsets (get-target self)) offset) (get-opcode self)))
(define (JMP target) (JMPcc target #xeb))
(define (JE  target) (JMPcc target #x74))
(define (JB  target) (JMPcc target #x72))
(define (resolve-jumps commands offsets)
  (define (iterate cmd acc)
    (let ((tail   (car acc))
          (offset (cdr acc)))
      (cond
        ((is-a? cmd <jmpcc>)  (cons (cons (resolve cmd (+ offset (len cmd)) offsets) tail)
                                    (+ offset (len cmd))))
