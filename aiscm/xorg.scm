...
(define-method (show (self <list>))
  (let* [(display (make <xdisplay>))
         ...
         (window  (cut make <xwindow> #:display display #:shape <> #:io IO-XIMAGE))
...
(define-method (show (self <procedure>))
  (let* [(img     (self))
    ...
    (title= window "AIscm")
    (show window (to-image img))
    (show window)
    (do () ((quit? display)) (set! img (self)) (show window (to-image img)) (process-events display))
    (hide window)
    (destroy display)
    img))
