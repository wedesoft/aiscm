...
(define (component self offset)
  (let* [(type (base (typecode self)))
         (size (size-of type))]
    (set-pointer-offset (pointer-cast type self) (* offset size))))
(define-method (red   (self <pointer<>>)) (component self 0))
(define-method (green (self <pointer<>>)) (component self 1))
(define-method (blue  (self <pointer<>>)) (component self 2))
(define (copy-rgb a b)
